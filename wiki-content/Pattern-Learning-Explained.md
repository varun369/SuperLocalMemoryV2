...
### Bayesian Confidence Scoring (v2.4.0)

SuperLocalMemory v2.4.0 replaced the frequency-based formula with a **Bayesian confidence model** grounded in published research on multi-agent collaborative learning.

**How it works:**
- Uses Bayesian posterior inference with pattern-specific priors tuned per category
- Log-scaled competition prevents confidence scores from collapsing as your memory database grows
- **Recency bonus**: Patterns observed recently get a boost that decays over time
- **Consistency bonus**: Patterns with high consistency across sessions get an additional boost
...
New formula:  Same data → 55% confidence (calibrated, actionable)
```
